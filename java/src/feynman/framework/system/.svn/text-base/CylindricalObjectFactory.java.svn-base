 ...
 * this class to create object that are used in the study of Cylindrical
 ...
 * PhysicalObjectFactoryClass=feynman.framework.CylindricalObjectFactory
 *
 * # User defined class of the PhysicalObject to use in the PhysicalSystem
 * PhysicalObjectClass=CylindricalParticle 
 * </pre>
 *
 * @author Wes Bailey
 * @version $Revision: 1.1.1.1 $ $Date: 2002/11/12 02:25:43 $
 */
public class CylindricalObjectFactory implements PhysicalObjectFactory {
	
	// Create an instance of the class.
	private static final CylindricalObjectFactory INSTANCE = new CylindricalObjectFactory();
	
	// Implement a singleton.
	private CylindricalObjectFactory() {
		// singleton
	}
	 ...
	 */
	public static final CylindricalObjectFactory getInstance() {
		return INSTANCE;
	}
	
	/** 
	 * Factory method for generating <b>PhysicalObject</b>s in the Cylindrical
	 * Coordinate System.
	 */
	public PhysicalObject create(Simulation sim) throws PhysicalObjectFactoryException {
		...
		try {
			...
			if (po instanceof CylindricalObject) {
				return po;
			} else {
				throw new PhysicalObjectFactoryException(clazz.getName() + "is not a CylindricalObject");
			}
		}
